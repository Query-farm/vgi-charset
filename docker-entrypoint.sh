...
#
# Dispatch the single vgi-charset image into one of its transports:
...
#
# The worker is stateless (pure chardetng/encoding_rs engines compiled in), so
...
case "${1:-http}" in
  http)
    ...
    exec charset-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec charset-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec charset-worker "$@"
