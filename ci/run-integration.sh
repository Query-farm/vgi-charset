...
#   VGI_CHARSET_WORKER  worker LOCATION the .test files attach (a stdio command
#                                       such as the compiled charset-worker binary, or an http:// URL)
# Optional:
#   STAGE             scratch dir for the preprocessed test tree (default: mktemp)
set -euo pipefail

: "${HAYBARN_UNITTEST:?path to the haybarn-unittest binary}"
: "${VGI_CHARSET_WORKER:?worker LOCATION (stdio command or http:// URL)}"
...
echo "Running suite (worker: $VGI_CHARSET_WORKER) ..."
