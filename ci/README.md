# CI: the vgi-charset worker integration suite
...
(`test/sql/*.test`) against the vgi-charset VGI worker through the **real DuckDB
...
| `TRANSPORT`  | `VGI_CHARSET_WORKER` (the ATTACH LOCATION) | how the worker is launched |
|--------------|------------------------------------------|----------------------------|
| `subprocess` | `…/target/release/charset-worker`          | DuckDB spawns the stdio binary (default) |
| `http`       | `http://127.0.0.1:<port>`                | `charset-worker --http` (auto port; prints `PORT:<n>` on stdout, which the script polls for) |
| `unix`       | `unix:///tmp/charset.<pid>.sock`           | `charset-worker --unix <sock>` (prints `UNIX:<sock>` on stdout + creates the socket; the script waits for both) |
...
1. **Build the worker** — `cargo build --release --bin charset-worker`. The
   compiled `target/release/charset-worker` is a self-contained stdio worker the
   extension spawns (the `.test` files `ATTACH` it via `${VGI_CHARSET_WORKER}`).
...
3. **Preprocess** — the standalone runner links none of the extensions the
   ...
   through untouched. (The vgi-charset `.test` files already `LOAD vgi;`
   explicitly and use `require-env VGI_CHARSET_WORKER`, so the `require`-rewrite is
...
4. **Run** — [`run-integration.sh`](run-integration.sh) brings up the worker for
   the selected `TRANSPORT` and sets `VGI_CHARSET_WORKER` accordingly, stages the
...
```bash
cargo build --release --bin charset-worker
...
WORKER_BIN="$PWD/target/release/charset-worker" \
TRANSPORT=subprocess \
  ci/run-integration.sh

# HTTP leg: launches `charset-worker --http`, reads PORT:<n>, attaches http://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/charset-worker" \
  TRANSPORT=http ci/run-integration.sh

# Unix leg: launches `charset-worker --unix /tmp/charset.<pid>.sock`, attaches unix://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/charset-worker" \
