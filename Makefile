# vgi-charset worker — dev, test, and lint targets.
#
# Usage:
...
WORKER         ?= $(CURDIR)/target/release/charset-worker
...
test-sql: build
	VGI_CHARSET_WORKER="$(WORKER)" $(SQL_RUNNER) --test-dir "$(TEST_DIR)" "$(TEST_PATTERN)"
