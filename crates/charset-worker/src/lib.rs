//! The `charset` VGI worker (library).
//!
...
//! the `charset-wasm` crate (the browser build, which serves the same `Worker`
//! over a SharedArrayBuffer byte channel instead).
//!
//! A standalone worker DuckDB launches and talks to over Apache Arrow IPC
...
/// The catalog name DuckDB sees in `ATTACH 'charset' (TYPE vgi, …)`. Defaults to
/// `charset`, but honors an explicit override so a test harness can rename it.
