#!/usr/bin/env bash
# Build the browser (`worker:`) build of the charset VGI worker:
# charset-wasm compiled to wasm32-unknown-emscripten, linked by emcc with the SAB
# ring js-library into a MODULARIZE'd Web Worker module.
#
...
#   - a nightly toolchain with the rust-src component and the
#     wasm32-unknown-emscripten target: -Z build-std is required because the
...
LIB="target/$TARGET/release/libcharset_wasm.a"
...
# which the Arrow encoders can overflow; an overflow surfaces as a bare "memory
# access out of bounds" with no console output. INITIAL_MEMORY must cover main
# stack + every pthread stack up front, or startup stalls growing shared memory
# while the pool is spawning.
echo "==> emcc link"
